...
It's also possible to pass a variable directly:

```vto
{{ set data = { size: "big" } }}

{{ layout "container.vto" data }}
  <h1>Hello, world!</h1>
{{ /layout }}
```
...
The `{{ default }}` tag allows to assign a value to a variable if the variable is `undefined` or `null`. This is useful to provide default content for the slots. Let's see an example with this layout:
...
We can pass the `header` variable or slot when using the layout:
...
But if we use the layout without a `header`, the default value is used instead:
