...
default Vento has the `escape`, `unescape`, `empty` and `safe` filters out of the box.

The `escape` filter allows to escape HTML entities:
...
The `empty` filter checks if the variable is empty and returns a boolean. A variable is considered empty if it meet any of the following conditions:
