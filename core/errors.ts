...
export function throwVentoSyntaxError(
  error: SyntaxError,
...
export function throwTagSyntaxError(
  error: SyntaxError,
  context: ErrorContext
): void {
  const token = context.tokens?.at(-1)
...
export async function throwJSSyntaxError(
  error: SyntaxError,
  context: ErrorContext
): Promise<void> {
...
function getAccurateErrorPosition(
  ...
  const token = tokens.findLast(token => token[2] <= position)
  if(!token) return -1
